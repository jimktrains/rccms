...
class Kohana_ORM {
	...
	public function find($id = NULL)
	{
		...
		if ($id !== NULL)
		{
			...
			$pk = $this->unique_key($id);
			$this->_db_builder->where($this->_table_name.'.'.$pk, '=', $id);
		}
		
		if(count($this->_changed)){
			foreach($this->_changed as $col=>$val){
				$this->_db_builder->where($this->_table_name.'.'.$col, '=', $this->_object[$col]);
			}
		}
	...
	public function errors($file, $translate=TRUE){
