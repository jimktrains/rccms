...
class DX_Auth
{
	...
	*/
	...
	function autologin()
	{
		...
		//if ($auto = $this->ci->input->cookie($this->ci->config->item('DX_autologin_cookie_name')) AND ! $this->ci->session->userdata('logged_in'))
	...
	*/
	...
	function is_role($roles = array(), $use_role_name = FALSE)
	{
		// Default return value
		$result = FALSE;
		...
		if ($use_role_name)
		{
			$get_function = 'get_role_name';
		}
		else
		{
			$get_function = 'get_role_id';
		}
		...
		{
			if (strtolower($this->$get_function()) == strtolower($val))
			{
				$result = TRUE;
				break;
			}
	...
	{
		// Trigger event
		$this->ci->dx_auth_event->user_logging_out($this->ci->session->userdata('user_id'));
	...
	function change_password($old_pass, $new_pass)
	{
		...
		if ($query = $this->ci->users->get_user_by_id($this->ci->session->userdata('user_id')) AND $query->num_rows() > 0)
		{
			...
			if (crypt($pass, $row->password) === $row->password)
			{
				...
				// Trigger event
				$this->ci->dx_auth_event->user_changed_password($this->ci->session->userdata('user_id'), $new_pass);
	...
	function cancel_account($password)
	{
		...
		if ($query = $this->ci->users->get_user_by_id($this->ci->session->userdata('user_id')) AND $query->num_rows() > 0)
		{
			...
			{
				// Trigger event
				$this->ci->dx_auth_event->user_canceling_account($this->ci->session->userdata('user_id'));
