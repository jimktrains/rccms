...
class Kohana_Request {
	...
	public function execute()
	{
		...
		try
		{
			...
			$this->action = empty($this->action) ? Route::$default_action : $this->action;

			$class->getMethod('before')->invoke($controller);
			if($class->hasMethod('action_'.$this->action)){	
				$class->getMethod('action_'.$this->action)->invokeArgs($controller, $this->_params);				

			}else{
				$req_method = strtolower(Request::$method);
				$this->_action_requested = $this->action;
				if($class->hasMethod($req_method . '_action_'.$this->action)){
					$class->getMethod($req_method . '_action_'.$this->action)->invokeArgs($controller, $this->_params);				
				}else{
					$this->_action_requested = $this->action;
					$this->action = 'not_found';
					$class->getMethod($this->action)->invokeArgs($controller, $this->_params);
				}
			}
		...
		catch (Exception $e)
		{
			...
			if ($e instanceof ReflectionException)
			{
				...
				$class = new ReflectionClass($prefix."Error");
				$controller = $class->newInstance($this);
				$class->getMethod('before')->invoke($controller);
				$class->getMethod('not_found')->invokeArgs($controller, $this->_params);				
				$class->getMethod('after')->invoke($controller);
				var_dump($e);
			}
			else
